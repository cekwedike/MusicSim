...
const getHintColor = (hint: 'safe' | 'risky' | 'dangerous'): string => {
    switch (hint) {
        case 'safe': return 'bg-semantic-success';
        case 'risky': return 'bg-semantic-warning';
        case 'dangerous': return 'bg-semantic-error';
    }
};
...
const ChoiceButton: React.FC<{
    ...
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className="w-full text-left p-3 sm:p-4 bg-secondary hover:bg-brand-primary border border-default rounded-lg transition-all duration-200 ease-in-out transform hover:scale-[1.02] sm:hover:scale-105 focus:outline-none focus:ring-2 focus:ring-brand-primary/50 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none relative min-h-[44px] sm:min-h-[56px] hover:text-white hover:border-brand-primary"
        >
            <p className="font-semibold pr-8 sm:pr-8 text-sm sm:text-base leading-relaxed text-primary">{choice.text}</p>
            {hint && (
                <div
...
const ScenarioCard: React.FC<ScenarioCardProps> = ({ scenario, onChoiceSelect, disabled, difficulty }) => {
    ...
    return (
        <div className="scenario-card card-elevated animate-fade-in max-w-2xl w-full">
            <h2 className="text-base sm:text-2xl md:text-3xl font-bold text-brand-primary mb-2 sm:mb-4 leading-tight">{scenario.title}</h2>
            ...
            <p className="text-secondary mb-3 sm:mb-6 leading-relaxed text-sm sm:text-base">{scenario.description}</p>
                ...
                <div className="mb-2 sm:mb-4 hidden sm:flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-muted">
                        <span className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-semantic-success"></div>
                            Safe
                        </span>
                        <span className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-semantic-warning"></div>
                            Risky
                        </span>
                        <span className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-semantic-error"></div>
                            Dangerous
                        </span>
                    </div>
                    <button
